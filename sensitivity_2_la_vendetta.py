...
# %%
pic_folder = '/home/emariott/deepmagic/notebooks/folder_14feb_13edge'
...
energy_sim = read_pkl('/home/emariott/deepmagic/output_data/for_sensitivity/energy_sim_test_point.pkl')
...
print("The total rate of simulated gamma events is %.4f Hz" % (R / N_))

# %%

e_w = ((e) ** (index_w - Index_sim)) * R / N_
e_trig_w = ((e_trig) ** (index_w - Index_sim)) * R / N_
...
time_obs_h = np.sum(df_h['timediff'])  # TODO: check the if from the scripts
...
ring_condition = np.logical_and(theta_2_h > (0.2) ** 2, theta_2_h < (0.53) ** 2)
...
def plot_MAGIC_Sensitivity(ax):
    s = np.loadtxt('/home/emariott/deepmagic/notebooks/magic_sensitivity.txt')
