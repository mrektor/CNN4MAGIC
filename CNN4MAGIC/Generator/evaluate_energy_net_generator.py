...
model = load_model(
    'output_data/checkpoints/Tranfer_Ensemble_SE_InceptionV3_SingleDense_energy_from40_last6_nofreeze_dense64_adam4e-4.hdf5')
# %%
model.load_weights(
...
def plot_gaussian_error(y_true, y_pred, net_name, fig_folder, num_bins=10, do_show=False, **kwargs):
    ...
    plt.legend([net_name, 'Aleksic et al. (2016)'])
...
folder_fig = '/home/emariott/deepmagic/output_data/pictures/for_paper/all_ensembles'
to_draw_list = pred_limate
legend = net_names  # ['Snapshot epoch 10', 'Snapshot epoch 11', 'Ensamble of epoch 10 and 11', 'Current State-of-the-Art']
plot_gaussian_error_2(energy_te_limato, to_draw_list, 'AllAdjustedPostRuben1',
...
def plot_improvement(y_true, y_pred, net_name, fig_folder, legend_list, num_bins=10, do_show=False, **kwargs):
    ######## PAPER DATA
    cutting_edge_magic_bins = [[47, 75],
                               [75, 119],
                               [119, 189],
    ...
    for i, pred in enumerate(y_pred):
        ...
        plt.semilogx(cutting_edge_magic_bins_median, sigma_enhancement, marker=marker_set[i])

    plt.grid(which='both', linestyle='--')
    plt.hlines(0, 10, 1e5, linestyles='-.')
    plt.xlim([30, 3e4])
    plt.title('Enhancement of $\sigma$ of linear prediction error w.r.t Aleksic et al. (2016)')
    plt.xlabel('Bin median value (GeV)')
    plt.ylabel('Enhancement (%)')
    plt.legend(legend_list)
    plt.tight_layout()
    plt.savefig(fig_folder + '/' + net_name + '_enhancement.png')
    plt.savefig(fig_folder + '/' + net_name + '_enhancement.pdf')


# %%
folder_fig = '/home/emariott/deepmagic/output_data/pictures/for_paper/enhancement'
to_draw_list = pred_limate
legend = net_names  # ['Snapshot epoch 10', 'Snapshot epoch 11', 'Ensamble of epoch 10 and 11', 'Current State-of-the-Art']
plot_improvement(energy_te_limato, to_draw_list, 'AllAdjustedPostRuben1',
                 fig_folder=folder_fig, legend_list=legend[:-1], num_bins=10,
                 do_show=False)
